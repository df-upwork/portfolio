...
#
Сейчас `Tᨀ` выровнен (`A𐏐`) по центру `Rᨀ` (и по вертикали, и по горизонтали) посредством правила `transform='translate(140.18362, 44.533066)'` (`Rⰳ`).

# Твоя задача
Предложи точечные правки к `Sᨀ`, чтобы `A𐏐` решалась без магических чисел типа `Rⰳ`.

# Половинчатое решение
`A𐏐` работает наполовину: только по горизонтали.
И `white-space:pre;` мне не нравится, это мешает удобному форматированию.
```svg
		<text
			class='common'
			inkscape:label='Text'
