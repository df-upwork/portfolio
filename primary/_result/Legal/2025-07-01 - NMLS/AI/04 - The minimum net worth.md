...
Def(`NW`):
```
«the minimum net worth» в контексте `S`
```

## 22. Твоя задача
1) Что такое вообще `NW`?
Как законодательство США определяет это понятие?
2) Перечисли в виде таблицы требование к `NW` в каждом из штатов.
У таблицы должно быть 2 колонки: название штата на английском и значение `NW` в виде числа (без символа валюты).
Таблица должна быть отформатирована разметкой Markdown.
Строки таблицы должны быть упорядочены по названию штата.
3) Обоснуй свой ответ ссылками на авторитетные источники: особенно на нормативные акты.
...
7) В своём анализе используй авторитетные источники информации на английском языке.
8) Свой ответ дай на русском языке. 
9) В своём ответе сошлись на конкретные пункты конкретных нормативных актов, с конкретными цитатами из них.
