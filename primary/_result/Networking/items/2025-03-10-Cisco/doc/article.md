## 1.
Connect `ge-0/0/1` (node0) with `ge-1/0/1` (node1) for the control link.
## 2.
Connect `ge-0/0/7` (node0) with `ge-1/0/7` (node1) for the fabric link.
## 3.
Connect 4G modems: I recommend 2 modems (one for each node) to physically provide 4G on both SRX devices.
## 4.
Connect multiple ports (for LAG) from each SRX300 to the Cisco Catalyst stack.
## 5.
```
...
### 6.1.
If the SRX is still in standalone mode, then enable the cluster:
```
...
### 7.1.
If the SRX is still in standalone mode, first enable the cluster (similar to 6.1):
```
